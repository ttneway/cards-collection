ALTER TABLE public.remote_ai_settings
  ADD COLUMN IF NOT EXISTS seed_mode TEXT NOT NULL DEFAULT 'random' CHECK (seed_mode IN ('random', 'fixed')),
  ADD COLUMN IF NOT EXISTS fixed_seed BIGINT;

DROP FUNCTION IF EXISTS public.get_remote_ai_settings();
DROP FUNCTION IF EXISTS public.upsert_remote_ai_settings(TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS public.upsert_remote_ai_settings(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BIGINT, BOOLEAN);
...
BEGIN
  IF public.current_user_role() NOT IN ('teacher', 'admin') THEN
    RAISE EXCEPTION 'Only teachers and admins can view shared AI settings.';
  END IF;
